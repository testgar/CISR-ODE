...
public:
            ...
            state_type &out , deriv_type &dxdt_out , time_type &dt)
    {
        ...
        if( max_rel_err > 1.0 )
        {
            ...
            dt *= max( value_type( value_type(9)/value_type(10) *
        ...
        else
        {
            ...
            {   //error too small - increase dt and keep the evolution and limit scaling factor to 5.0
                // error should be > 0
                max_rel_err = max( value_type( pow( value_type(5.0) , -value_type(m_stepper.stepper_order_value) ) ) ,
                                                                     max_rel_err );
                t += dt;
...
public:
    ...
    controlled_step_result try_step( System system , const state_type &in , const deriv_type &dxdt_in , time_type &t , state_type &out , time_type &dt )
    {
        ...
        else
        {
            ...
            {
                // error should be > 0
                m_max_rel_error = max ( value_type( pow( value_type(5.0) , -value_type(m_stepper.stepper_order_value) ) ) ,
                            m_max_rel_error );
                //error too small - increase dt and keep the evolution and limit scaling factor to 5.0
                t += dt;
