...
echo "">>$out_file
echo "namespace inputs">>$out_file
echo "{">>$out_file

vector=($(./scripts/read_column.sh 0 $in_file))
for index in ${!vector[*]}
do
	echo -e "\t// i$index\t${vector[index]//\"/}">>$out_file
done
echo "">>$out_file
for index in ${!vector[*]}
do
...
echo "">>$out_file
echo -e "\t// total size of inputs vector">>$out_file
echo -e "\tconst int input_size=${#vector[@]};">>$out_file
echo "}">>$out_file
